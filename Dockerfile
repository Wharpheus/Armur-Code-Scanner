...
RUN curl -OL https://golang.org/dl/go1.23.0.linux-arm64.tar.gz && \
    tar -C /usr/local -xvf go1.23.0.linux-arm64.tar.gz && \
...
# Install gosec
RUN /usr/local/go/bin/go install github.com/securego/gosec/v2/cmd/gosec@v2.20.0
...
## Install gocyclo
RUN /usr/local/go/bin/go install github.com/fzipp/gocyclo/cmd/gocyclo@latest
#
## Install deadcode
RUN /usr/local/go/bin/go install golang.org/x/tools/cmd/deadcode@latest
#
## Install osv-scanner
...
# Copy the ESLint configuration file to the appropriate directory
...
# Ensure gosec is available in the PATH
